...
import numpy as np
from tools_prune import common as common
from tools_prune import algo as algo
import os
...
def weight_pruner_loader(pruner_string, custom_data = None):
    """
    ...
    if pruner_string == "lamp":
        ...
        return RDPruner(custom_data = custom_data)
    else:
...
def prune_weights_l1predefined(model, amounts, only_layerids=None):
    ...
    for idx, m in enumerate(mlist):
        if only_layerids is not None and idx not in only_layerids:
            continue
        if isinstance(m, spnn.Conv3d):
            prune.l1_unstructured(m, name="kernel", amount=float(amounts[idx]))
        else:
...
def prune_weights_l1structured(model, amounts, only_layerids=None):
    ...
    for idx, m in enumerate(mlist):
        ...
        if isinstance(m, spnn.Conv3d):
            prune.ln_structured(m, name="kernel", amount=float(amounts[idx]), n = 1, dim = 1)
        else:
            prune.ln_structured(m, name="weight", amount=float(amounts[idx]), n = 1, dim = 1)
...
def gen_rd_curves(net, loader, args, prefix=None, suffix=None, custom_data = None):
    ...
    ########
        ...
        dummy_input = custom_data
    ##########

    _ = net(next(iter(dummy_input)))
    ...
    for l in range(0, len(layers)):
        if isinstance(layers[l], spnn.Conv3d):
            layer_weights = layers[l].kernel.clone()
        else:
    ...
    # print(nchannels, n_channel_elements)
    net.eval()
    if args.dataset == "cifar":
        # Y, labels = common.predict2_withgt(net, loader)
        Y, labels = common.predict2_withgt(net, custom_data)
    else:
    ...
    with torch.no_grad():
        for layerid in range(len(layers)):
            ...
            ##################################
            if isinstance(layers[layerid], spnn.Conv3d):
                layer_weights = layers[layerid].kernel.clone()
            else:
            ...
            for d in range(args.maxsps + 1): # 100
                amount = (1.0 - min_amount) * d / args.maxsps + min_amount      # amount to get pruned
                rst_amount[d] = amount

                ##########################
                if isinstance(layers[layerid], spnn.Conv3d):
                    prune_weights = algo.pruning(layers[layerid].kernel, amount)
                    layers[layerid].kernel.data = prune_weights
                else:
                ...
                if args.dataset == "cifar":
                    ...
                    Y_hat = common.predict2(net, dummy_input)
                else:
                ...
                if isinstance(layers[layerid], spnn.Conv3d):
                    layers[layerid].kernel.data = layer_weights
                else:
...
class RDPruner:
    def __init__(self, custom_data = None):
        ...
        ############
    def __call__(self, model, amount, args, loader, container, to_prune_layerids=None, epoch_cnt=0, custom_data = None):
        ...
        if args.synth_data:
            rd_dist, rd_phi = gen_rd_curves_synth_data(
                container,
                args,
                prefix=f"./rd_retrain/weight_rewind_{args.weight_rewind}/{args.seed}/remask_per_iter_{args.remask_per_iter}/{(args.prune_mode).upper()}/ranking_{args.ranking}/",
        ...
        dp_save_path = f"./rd_retrain/weight_rewind_{args.weight_rewind}/{args.seed}/remask_per_iter_{args.remask_per_iter}/{(args.prune_mode).upper()}/sp{target_sparsity}_{args.model}_ndz_{args.maxsps:04d}_rdcurves_ranking_{args.ranking}_opt_dist_dp.joblib"
        g = None
        if os.path.exists(dp_save_path):
            # p = pickle.load(open(dp_save_path, "rb"))
            import joblib
            p = joblib.load(dp_save_path)
            g = p["g"]
        ...
        # print(dp_save_path, type(dp), type(g))
        # print(dp,'\n\n*******\n\n',g)
        ...
        if not os.path.exists(dp_save_path):
            ...
            joblib.dump({"dp": dp, "g": g}, dp_save_path, compress = 9, protocol = 4)
        ...
        mask_save_path = f"./rd_retrain/weight_rewind_{args.weight_rewind}/{args.seed}/remask_per_iter_{args.remask_per_iter}/{(args.prune_mode).upper()}/sp{target_sparsity}_{args.model}_ndz_{args.maxsps:04d}_rdcurves_ranking_{args.ranking}_opt_dist_mask.pt"
